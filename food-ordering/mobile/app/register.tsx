...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    if (role === 'restaurant' && (!phone || !location)) {
      setError('Please provide phone and location details for the restaurant.');
      return;
    }
    setError('');
    ...
    try {
      ...
      if (result?.otpRequired) {
        Alert.alert('Verification Sent', '📧 A 6-digit OTP code has been sent to your email.');
        router.push('/verify-otp');
      } else {
        Alert.alert('Registration Complete', '✅ Your account has been registered successfully. Please log in.');
      ...
      >
        <ScrollView contentContainerStyle={styles.container}>
          ...
          <View style={[styles.authBox, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.subtext, { color: colors.subtext }]}>Create an account to get started</Text>

            {/* Toggle between Login and Signup */}
            <View style={[styles.toggleContainer, { backgroundColor: colors.toggleBg }]}>
              <TouchableOpacity style={styles.toggleBtn} onPress={() => router.replace('/login')}>
                <Text style={[styles.toggleBtnText, { color: colors.roleTabInactiveText }]}>Login</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.toggleBtn, { backgroundColor: colors.roleTabActiveBg }]}>
                <Text style={{ fontSize: 16, fontWeight: '600', color: colors.roleTabActiveText }}>Sign Up</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={[styles.roleContainer, { backgroundColor: colors.roleTabBg, borderColor: colors.roleTabBorder }]}>
              ...
              >
                <Text style={[styles.roleTabText, { color: role === 'user' ? colors.roleTabActiveText : colors.roleTabInactiveText }]}>Student/Prof</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.roleTab, role === 'restaurant' && { backgroundColor: colors.roleTabActiveBg }]} 
                onPress={() => setRole('restaurant')}
              >
                <Text style={[styles.roleTabText, { color: role === 'restaurant' ? colors.roleTabActiveText : colors.roleTabInactiveText }]}>Restaurant Manager</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.form}>
              ...
              {role === 'restaurant' && (
                <>
                  <Text style={[styles.label, { color: colors.text }]}>Contact Phone Number</Text>
                  <TextInput
                    style={[styles.input, { backgroundColor: colors.inputBg, borderColor: colors.inputBorder, color: colors.inputText }]}
                  ...
                  <Text style={[styles.label, { color: colors.text }]}>Outlet Location</Text>
                  <TextInput
                    style={[styles.input, { backgroundColor: colors.inputBg, borderColor: colors.inputBorder, color: colors.inputText }]}
                  ...
                  <Text style={[styles.label, { color: colors.text }]}>Outlet Description</Text>
                  <TextInput
                    style={[styles.input, styles.textArea, { backgroundColor: colors.inputBg, borderColor: colors.inputBorder, color: colors.inputText }]}
                    placeholder="Describe your cuisine, specialties, or business hours..."
                    placeholderTextColor="#64748b"
                    multiline
                    numberOfLines={3}
                    value={description}
              ...
              >
                {isLoading ? (
                  ...
                  <Text style={[styles.submitBtnText, { color: colors.submitBtnText }]}>Create Account</Text>
                )}
              </TouchableOpacity>
            </View>
          </View>
