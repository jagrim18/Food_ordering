...
function RestaurantDashboard() {
  ...
  const getNextStatus = (s) => {
    const map = { pending: "Accepted", accepted: "Preparing", preparing: "Delivered" };
    return map[s?.toLowerCase()] || null;
  };

  const getButtonText = (s) => {
    const st = s?.toLowerCase();
    if (st === "pending") return "Accept Order";
    if (st === "accepted") return "Start Preparing";
    if (st === "preparing") return "Complete Order";
    return "";
  };

  // ---------- Sort Orders ----------
  const sortedOrders = [...orders].sort((a, b) => {
    const flow = ["pending", "accepted", "preparing", "delivered", "cancelled"];
    return flow.indexOf(a.status.toLowerCase()) - flow.indexOf(b.status.toLowerCase());
  });

  // ---------- FIX: Declare required variables ----------
  const activeOrdersList = sortedOrders.filter((o) =>
    ["pending", "accepted", "preparing"].includes(o.status?.toLowerCase())
  );
  ...
  const handleExport = async () => {
    try {
      ...
      if (res.data?.filePath) {
        ...
        window.open(base + res.data.filePath, "_blank");
      }
    } catch (err) {
      alert("Export failed.");
    } finally {
  ...
  // -------------------- RENDER -----------------------
  return (
    <div className="restaurant-dashboard" data-theme={localStorage.getItem("theme") || "light"}>
      <header className="dashboard-top">
        <h1 className="dashboard-title">
          {restaurant ? `${restaurant.name} Dashboard` : "Restaurant Dashboard"}
        </h1>
      ...
      <div className="orders-list">
        {loading ? (
          ...
          <p className="no-orders">{activeTab === "active" ? "No active orders." : "No past orders."}</p>
        ) : (
          displayOrders.map((order) => (
            <div className="order-card" key={order._id}>
              <div className="order-header">
                ...
                <div className="order-price-time">
                  <p>₹{(order.totalPrice || order.total || 0).toFixed(2)}</p>
                  <span>
                    {new Date(order.createdAt).toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
